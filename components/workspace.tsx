...
function GlobalRail({ accountName, artifacts, collapsed, onToggle, onNew, onArtifact }: { accountName: string; artifacts: Artifact[]; collapsed: boolean; onToggle: () => void; onNew: () => void; onArtifact: (type: Artifact["type"]) => void }) {
  return (
    <aside className={`global-rail ${collapsed ? "collapsed" : ""}`}>
      <div className="brand">{!collapsed && <strong>ContextSE</strong>}<button className="collapse-control" onClick={onToggle} aria-label={collapsed ? "Expand workspace rail" : "Collapse workspace rail"}>{collapsed ? <ChevronRight /> : <ChevronLeft />}</button></div>
  ...
  return (
    <aside className={`testing-lab ${collapsed ? "collapsed" : ""}`}>
      ...
      {!collapsed && <div className="testing-body">
        ...
        {result && result.error && <div className="model-error"><strong>Agent configuration needed</strong><p>{result.reaction}</p></div>}
  ...
  return (
    <div className="workspace-shell" style={layoutStyle}>
      ...
      <main className="agent-pane">
        <header className="agent-header"><div><span className="agent-mark"><Bot /></span><span><strong>ContextSE Agent</strong><small>{workspace.sellerCompany} → {workspace.name} · {workspace.sources.length} files</small></span></div><span className={`connection ${backendMode}`}><i />{backendMode === "convex" ? "AI connected" : "Demo AI connected"}</span></header>
